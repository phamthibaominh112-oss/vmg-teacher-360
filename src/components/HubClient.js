...
import { ROLE_LABELS, DELETE_ROLES, USER_ADMIN_ROLES, REGION_CENTRES, HEAD_OFFICE_ROLES, REGION_SCOPE_ROLES, CENTRE_SCOPE_ROLES, INCIDENT_CATEGORIES, OBSERVATION_RUBRIC, scoreBand, incidentRisk, KPI_REVENUE_TARGET, kpiBand, retentionPct, weightedKpiComposite } from '@/lib/config'

const NAV = {
  teacher:['overview','portfolio','kpi','framework','growth','observations','training','resources','bulletin','handbook','documents','support'],
  cmo:['overview','teachers','performance','kpi','framework','incidents','training','resources','touchpoints','upgrades','documents','support'],
  centre_director:['overview','teachers','performance','kpi','framework','incidents','training','resources','touchpoints','upgrades','documents','support'],
  regional_director:['overview','teachers','performance','kpi','framework','incidents','training','resources','touchpoints','upgrades','documents','support'],
  csr:['overview','teachers','incidents','coordination','training','resources','touchpoints','performance','framework','documents','support'],
  rnd:['overview','teachers','performance','kpi','framework','observations','training','touchpoints','upgrades','incidents','documents','support','resources','bulletin','access','imports','governance'],
  bod:['overview','teachers','performance','kpi','framework','observations','training','touchpoints','upgrades','incidents','documents','support','resources','bulletin','access','imports','governance'],
  academic_supervisor:['overview','teachers','performance','kpi','framework','observations','training','calendar','touchpoints','upgrades','documents','support','resources','imports'],
  ptns:['overview','teachers','documents','training','bulletin','support','kpi','framework','performance','touchpoints','resources','imports']
}
const GLYPHS={overview:'⌂',teachers:'◎',portfolio:'◉',framework:'≡',growth:'▦',resources:'⌕',support:'?',bulletin:'▤',training:'△',calendar:'□',observations:'✓',incidents:'!',performance:'↗',touchpoints:'↻',upgrades:'↑',documents:'▱',kpi:'%',coordination:'⇄',access:'◇',governance:'◆',imports:'⇩',handbook:'§'}
const LABEL_KEY={overview:'overview',teachers:'teachers',portfolio:'myPortfolio',framework:'framework',growth:'growthMatrix',resources:'resources',support:'support',bulletin:'bulletin',training:'training',calendar:'calendar',observations:'observations',incidents:'incidents',performance:'performance',touchpoints:'touchpoints',upgrades:'upgrades',documents:'documents',kpi:'kpi',coordination:'coordination',access:'access',governance:'governance',imports:'imports',handbook:'handbook'}
...
export default function HubClient({profile}){
  ...
  const load=useCallback(async()=>{
    ...
    const queries=[
      ...
      supabase.from('observations').select('*').order('observed_at',{ascending:false}).limit(2000),
      supabase.from('incidents').select('*').order('created_at',{ascending:false}).limit(2000),
      supabase.from('teacher_touchpoints').select('*').order('touch_date',{ascending:false}).limit(2000),
      ...
      supabase.from('teacher_kpi_snapshots').select('*').order('period_start',{ascending:false}).limit(2000),
    ...
    next.teachers=(next.users||[]).filter(u=>u.role==='teacher'&&u.is_active!==false)
    setData(next)
  ...
  function navGroups(){
    const defs=[
      ...
      {key:'people',label:t(lang,'menuPeople'),items:['growth','performance','framework','observations','training','calendar','touchpoints','upgrades']},
      {key:'operations',label:t(lang,'menuOperations'),items:['incidents','coordination','handbook','documents','support','kpi']},
  ...
  function renderView(){
    const map={overview:<Overview/>,portfolio:<Portfolio/>,growth:<GrowthMatrix/>,resources:<Resources/>,support:<Support/>,teachers:<Teachers/>,bulletin:<Bulletin/>,training:<Training/>,calendar:<Calendar/>,observations:<Observations/>,incidents:<Incidents/>,performance:<Performance/>,framework:<EvaluationFramework/>,touchpoints:<Touchpoints/>,upgrades:<Upgrades/>,documents:<Documents/>,kpi:<Kpi/>,coordination:<Coordination/>,access:<Access/>,imports:<Imports/>,governance:<Governance/>,handbook:<Handbook/>}
  ...
  function Overview(){
    ...
    if(teacherMode){
      ...
      const monthKpi=monthlyKpiIntel(teacher,nowDate().slice(0,7))
      return <div className="portfolio-dashboard">
        ...
        <section className="teacher-monthly-accountability">
          <div><span>{bi(lang,'MONTHLY KPI','KPI THÁNG')}</span><KpiBandPill band={monthKpi.band} score={monthKpi.score} completeness={monthKpi.completeness} lang={lang}/></div>
          <div><span>{bi(lang,'LEARNER BASE','HV ĐẦU KỲ')}</span><b>{monthKpi.learnerStart||'—'}</b></div>
          <div><span>{bi(lang,'DROPOUTS','HV RỤNG')}</span><b>{monthKpi.dropouts??'—'}</b><small>{monthKpi.learnerStart!=null?`${monthKpi.hvr??'—'}% HVR`:bi(lang,'Awaiting learner base','Chờ nhập số HV đầu kỳ')}</small></div>
          <div><span>{bi(lang,'APPROVED ISSUES','LỖI ĐÃ DUYỆT')}</span><b>{monthKpi.issues.approved}</b><small>{monthKpi.issues.pending} {bi(lang,'pending','chờ duyệt')}</small></div>
          <div><span>{bi(lang,'PENALTY','PHẠT')}</span><b className="penalty-off">{monthKpi.penaltyActive?Number(monthKpi.penaltyAmount).toLocaleString():bi(lang,'NOT APPLIED','CHƯA ÁP DỤNG')}</b><button onClick={()=>setView('kpi')}>{bi(lang,'View monthly KPI','Xem KPI tháng')} →</button></div>
        </section>
        <DashboardAnnouncements items={data.announcements.slice(0,3)} lang={lang} onOpen={()=>setView('bulletin')}/>
        <button className="standards-quicklink" onClick={()=>setView('handbook')}><span>§</span><div><b>{bi(lang,'Teacher Standards & Handbook','Quy định & Nội quy Giáo viên')}</b><small>{bi(lang,'Clear expectations for teaching quality, conduct, evidence, schedules and how issues are handled fairly.','Quy định rõ về chất lượng giảng dạy, tác phong, minh chứng, lịch làm việc và cách xử lý sự vụ công bằng.')}</small></div><i>→</i></button>
  ...
  function Teachers(){
    ...
    <div className="panel"><div className="table-wrap"><div className="table"><div className="tr th teacher-cols-v16"><span>{t(lang,'teachers')}</span><span>{t(lang,'centre')}</span><span>{t(lang,'score')}</span><span>HVR</span><span>{bi(lang,'Monthly KPI','KPI tháng')}</span><span>{bi(lang,'Approved issues','Lỗi đã duyệt')}</span><span>{t(lang,'recommendation')}</span></div>{visible.map(x=>{const intel=teacherIntel(x);return <div className="tr teacher-cols-v16" key={x.id} onDoubleClick={()=>{setSelectedTeacherId(x.id);setView('portfolio')}}><span className="teacher-cell"><AvatarPic user={x}/><span><b>{x.full_name}</b><small>{x.teacher_code||'—'} · {x.professional_level||'—'}</small></span></span><span><b>{x.home_centre_code||'—'}</b><small>{bi(lang,'Region','Khu vực')} {x.region_no||'—'}</small></span><span><b>{intel.score||'—'}</b><small>{intel.score?bandLabel(lang,intel.score):bi(lang,'No official score','Chưa có điểm chính thức')}</small></span><span><b>{intel.hvr?`${intel.hvr}%`:'—'}</b><small>{intel.learnerStart!=null?`${intel.dropouts??0}/${intel.learnerStart} ${bi(lang,'dropouts','rụng')}`:bi(lang,'No learner base','Chưa có mẫu HV')}</small></span><span><KpiBandPill band={intel.kpiBand} score={intel.kpiScore} completeness={intel.kpiCompleteness} lang={lang}/></span><span><b>{intel.approvedCases}</b><small>{intel.pendingCases} {bi(lang,'pending','chờ duyệt')} · {bi(lang,'Penalty not applied','Chưa áp dụng phạt')}</small></span><span><b>{intel.recommendation}</b><small>{bi(lang,'Double-click for full portfolio','Nhấp đúp để mở hồ sơ phát triển')}</small></span></div>})}{!visible.length&&<Empty lang={lang}/>}</div></div></div></>
  }
  ...
  function Training(){
    ...
    <div className="panel"><div className="table-wrap"><div className="table"><div className="tr th training-cols"><span>{bi(lang,'Date / time','Ngày / giờ')}</span><span>{bi(lang,'Development event','Hoạt động phát triển')}</span><span>{bi(lang,'Type','Loại')}</span><span>{bi(lang,'Audience','Đối tượng')}</span><span>{bi(lang,'Materials','Học liệu')}</span><span>{t(lang,'action')}</span></div>{rows.map(r=>{const reg=data.registrations.find(x=>x.training_id===r.id&&x.user_id===profile.id);const booked=Boolean(reg);return <div className="tr training-cols" key={r.id}><span><b>{fmt(r.starts_at,lang)}</b><small>{new Date(r.starts_at).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'})} · {relative(r.starts_at,lang)}</small></span><span><b>{r.title}</b><small>{r.reason||r.description||r.location||bi(lang,'VMG development','Phát triển chuyên môn VMG')}</small></span><span><span className="pill gold">{dv(lang,r.training_type)}</span></span><span><b>{dv(lang,r.audience_label||'Targeted teachers')}</b><small>{r.target_region_no?`${bi(lang,'Region','Khu vực')} ${r.target_region_no}`:bi(lang,'All / selected','Tất cả / đã chọn')}</small></span><span><b>{r.slides_path?t(lang,'slides'):'—'}</b><small>{r.recap_text?t(lang,'recap'):bi(lang,'Recap pending','Chưa có recap')}</small></span><span className="row-actions">{profile.role==='teacher'&&<button className={`btn small ${booked?'secondary':'primary'}`} disabled={booked} onClick={()=>bookTraining(r.id)}>{booked?t(lang,'booked'):t(lang,'book')}</button>}{profile.role==='teacher'&&booked&&<CalendarSyncChip lang={lang} status={reg.calendar_sync_status} onRetry={()=>syncGoogleCalendar('training_registration',reg.id).then(()=>afterAction())}/>} {r.slides_path&&<button className="btn small secondary" onClick={()=>openMaterial(r.slides_path,'training-materials')}>{t(lang,'slides')}</button>}{r.recap_text&&<button className="btn small secondary" onClick={()=>setModal({type:'recap',record:r})}>{t(lang,'recap')}</button>}</span></div>})}{!rows.length&&<Empty lang={lang}/>}</div></div></div></>
  }

  function Calendar(){
    const bookedTrainingIds=new Set(data.registrations.filter(x=>x.user_id===profile.id&&x.status==='booked').map(x=>x.training_id))
    const all=[...data.trainings.filter(x=>profile.role!=='teacher'||bookedTrainingIds.has(x.id)).map(x=>({...x,kind:'Training',calendarRegistration:data.registrations.find(r=>r.training_id===x.id&&r.user_id===profile.id)})),...data.events.map(x=>({...x,kind:x.event_type||'Event'}))].filter(x=>profile.role!=='teacher'||!x.teacher_id||x.teacher_id===profile.id).sort((a,b)=>new Date(a.starts_at)-new Date(b.starts_at))
    const canBook=['academic_supervisor','cmo','centre_director','regional_director','rnd','bod'].includes(profile.role)
    return <><PageIntro kicker={bi(lang,'YOUR DEVELOPMENT CALENDAR','LỊCH PHÁT TRIỂN CỦA BẠN')} title={t(lang,'calendar')} text={lang==='vi'?'Đào tạo, trao đổi định kỳ, đào tạo lại và họp giáo viên. Khi đã book, lịch có thể tự động đồng bộ vào Google Calendar cá nhân và vẫn được nhắc trước 24 giờ.':'Training, catch-up, retraining and teacher meetings. Confirmed bookings can sync automatically to the individual Google Calendar and still receive a 24-hour reminder.'} actions={canBook&&<button className="btn primary" onClick={()=>setModal({type:'event'})}>＋ {bi(lang,'Add catch-up / meeting','Thêm lịch trao đổi / họp')}</button>}/><div className="grid12">{all.map(e=>{const sync=e.kind==='Training'?e.calendarRegistration:e;const retryType=e.kind==='Training'?'training_registration':'teacher_event';const retryId=e.kind==='Training'?e.calendarRegistration?.id:e.id;return <div className="panel span6" key={`${e.kind}-${e.id}`}><div className="panel-head"><div><span>{bi(lang,e.kind.toUpperCase(),e.kind==='Training'?'ĐÀO TẠO':'LỊCH GIÁO VIÊN')} · {relative(e.starts_at,lang)}</span><h3>{e.title}</h3></div><span className="pill gold">{fmt(e.starts_at,lang)}</span></div><div className="panel-body"><div className="facts" style={{gridTemplateColumns:'repeat(4,1fr)',marginTop:0}}><div><span>{bi(lang,'Time','Giờ')}</span><b>{new Date(e.starts_at).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'})}</b></div><div><span>{bi(lang,'Location','Địa điểm')}</span><b>{e.location||'TBC'}</b></div><div><span>{bi(lang,'Reminder','Nhắc lịch')}</span><b>T−24h</b></div><div><span>Google Calendar</span>{e.kind==='Training'&&profile.role!=='teacher'?<b>{bi(lang,'Per booking','Theo từng lượt book')}</b>:<CalendarSyncChip lang={lang} status={sync?.calendar_sync_status} onRetry={retryId?()=>syncGoogleCalendar(retryType,retryId).then(()=>afterAction()):null}/>}</div></div></div></div>})}{!all.length&&<div className="panel span12"><Empty lang={lang}/></div>}</div></>
  }
  ...
  function Incidents(){
    ...
    const teacherMode=profile.role==='teacher'
    const rows=teacherMode?data.incidents.filter(x=>x.teacher_id===profile.id):data.incidents
    const ownMonth=teacherMode?monthlyKpiIntel(currentUser,nowDate().slice(0,7)):null
    ...
    {teacherMode&&ownMonth&&<div className="teacher-issue-summary"><div><span>{bi(lang,'APPROVED THIS MONTH','LỖI ĐÃ DUYỆT THÁNG NÀY')}</span><b>{ownMonth.issues.approved}</b></div><div><span>{bi(lang,'PENDING REVIEW','CHỜ DUYỆT')}</span><b>{ownMonth.issues.pending}</b></div><div><span>{bi(lang,'CRITICAL','NGHIÊM TRỌNG')}</span><b>{ownMonth.issues.critical}</b></div><div><span>{bi(lang,'PENALTY','PHẠT')}</span><b className="penalty-off">{ownMonth.penaltyActive?Number(ownMonth.penaltyAmount).toLocaleString():bi(lang,'NOT APPLIED','CHƯA ÁP DỤNG')}</b><small>{bi(lang,'Counts are visible for transparency only.','Hiện chỉ hiển thị số liệu minh bạch.')}</small></div></div>}
  ...
  function Performance(){
    ...
    const avg=scored.length?Number((scored.reduce((a,x)=>a+Number(x.score),0)/scored.length).toFixed(1)):null
    const approvedIssues=rows.reduce((a,x)=>a+x.issues.approved,0)
    const dropouts=rows.reduce((a,x)=>a+Number(x.dropouts??0),0)
    const completeness=rows.length?Math.round(rows.reduce((a,x)=>a+Number(x.completeness||0),0)/rows.length):0
    const canEditKpi=['cmo','centre_director','regional_director','ptns','rnd','bod'].includes(profile.role)
    const actions=<div className="page-action-row"><input className="month-picker" type="month" value={month} onChange={e=>setMonth(e.target.value)}/>{canEditKpi&&<button className="btn secondary" onClick={()=>setModal({type:'kpi',month})}>＋ {bi(lang,'Record monthly data','Nhập dữ liệu tháng')}</button>}<button className="btn primary" onClick={()=>exportMonthlyKpi(month,data.teachers)}>⇩ {bi(lang,'Export monthly KPI','Xuất KPI tháng')}</button></div>
    return <><PageIntro kicker={bi(lang,'DATA → RUBRIC → DECISION','DỮ LIỆU → RUBRIC → QUYẾT ĐỊNH')} title={t(lang,'performance')} text={bi(lang,'Monthly teacher performance is now tied to explicit data sources, formulas and bands. No vague “stable” label is used as a KPI result.','Hiệu suất giáo viên theo tháng được gắn với nguồn dữ liệu, công thức và thang xếp loại cụ thể. Không dùng nhãn “Đang ổn” mơ hồ làm kết quả KPI.')} actions={actions}/>
      <div className="performance-kpi-strip"><div><span>{bi(lang,'Teachers in scope','GV trong phạm vi')}</span><b>{rows.length}</b></div><div><span>{bi(lang,'Average KPI','KPI trung bình')}</span><b>{avg??'—'}</b></div><div><span>{bi(lang,'Dropouts recorded','HV rụng ghi nhận')}</span><b>{dropouts}</b></div><div><span>{bi(lang,'Approved issues','Lỗi đã duyệt')}</span><b>{approvedIssues}</b></div><div><span>{bi(lang,'Data completeness','Độ đầy đủ dữ liệu')}</span><b>{completeness}%</b></div></div>

      <section className="kpi-rubric-shell">
        <div className="kpi-rubric-head"><div><span>{bi(lang,'MONTHLY KPI MATRIX · OPERATING RUBRIC V1','MA TRẬN KPI THÁNG · RUBRIC VẬN HÀNH V1')}</span><h3>{bi(lang,'Every number has a source and a calculation','Mỗi con số đều có nguồn và cách tính')}</h3></div><span className="draft-policy-badge">{bi(lang,'DRAFT POLICY · R&D/BOD APPROVAL REQUIRED','DỰ THẢO · CẦN R&D/BOD PHÊ DUYỆT')}</span></div>
        <div className="kpi-rubric-grid">
          <KpiRubricMetric code="01" title={bi(lang,'Observation quality','Chất lượng dự giờ')} weight="50%" source={bi(lang,'Official VMG observation','Phiếu dự giờ VMG chính thức')} formula={bi(lang,'Recorded /100 score; monthly average if multiple observations exist.','Điểm /100; nếu có nhiều lần dự giờ trong tháng thì lấy trung bình.')} detail={bi(lang,'>90 Excellent · 81–90 Very good · 70–80 Good · 60–69 Satisfactory · <60 Development/Training','>90 Xuất sắc · 81–90 Rất tốt · 70–80 Tốt · 60–69 Đạt · <60 Cần cải thiện/đào tạo')}/>
          <KpiRubricMetric code="02" title={bi(lang,'Learner retention · HVR','Duy trì học viên · HVR')} weight="30%" source={bi(lang,'Learners at start + dropouts','HV đầu kỳ + số HV rụng')} formula={bi(lang,'(Learners at start − dropouts) ÷ learners at start × 100','(HV đầu kỳ − HV rụng) ÷ HV đầu kỳ × 100')} detail={bi(lang,'HVR is calculated automatically; no manual percentage is required for new monthly records.','HVR được hệ thống tự tính; dữ liệu tháng mới không cần nhập % thủ công.')}/>
          <KpiRubricMetric code="03" title={bi(lang,'Revenue contribution','Đóng góp doanh thu')} weight="20%" source={bi(lang,'Monthly recorded contribution','Doanh thu đóng góp trong tháng')} formula={bi(lang,`Index = revenue ÷ ${KPI_REVENUE_TARGET.toLocaleString()} × 100; capped at 100`,`Chỉ số = doanh thu ÷ ${KPI_REVENUE_TARGET.toLocaleString()} × 100; tối đa 100`)} detail={bi(lang,'The current target is configurable and is not a permanent policy constant.','Mốc hiện tại có thể cấu hình và chưa phải hằng số chính sách.')}/>
          <KpiRubricMetric code="04" title={bi(lang,'Approved issues','Lỗi / sự vụ đã duyệt')} weight="0%" source={bi(lang,'Director-approved incident records','Sự vụ đã được GĐTT phê duyệt')} formula={bi(lang,'Counted monthly and shown to the teacher; pending cases are separated.','Đếm theo tháng và hiển thị cho GV; sự vụ chờ duyệt được tách riêng.')} detail={bi(lang,'Penalty policy is NOT active. Issue counts do not deduct KPI at this stage.','CHƯA áp dụng chính sách phạt. Số lỗi hiện chưa bị trừ vào KPI.')}/>
        </div>
  ...
  function Kpi(){
    const [month,setMonth]=useState(nowDate().slice(0,7))
    const teacherMode=profile.role==='teacher'
    const teachers=teacherMode?[currentUser]:data.teachers
    const rows=teachers.map(teacher=>({teacher,...monthlyKpiIntel(teacher,month)}))
    const canEditKpi=['cmo','centre_director','regional_director','ptns','rnd','bod'].includes(profile.role)
    const actions=<div className="page-action-row"><input className="month-picker" type="month" value={month} onChange={e=>setMonth(e.target.value)}/><button className="btn soft" onClick={()=>setView('framework')}>≡ {teacherMode?bi(lang,'How am I evaluated?','Tôi được đánh giá thế nào?'):bi(lang,'Evaluation framework','Khung đánh giá')}</button>{canEditKpi&&<button className="btn secondary" onClick={()=>setModal({type:'kpi',month})}>＋ {bi(lang,'Record KPI','Ghi nhận KPI')}</button>}<button className="btn primary" onClick={()=>exportMonthlyKpi(month,teachers)}>⇩ {teacherMode?bi(lang,'Export my KPI','Xuất KPI của tôi'):bi(lang,'Export monthly KPI','Xuất KPI tháng')}</button></div>
    const own=rows[0]
    return <><PageIntro kicker={bi(lang,'MONTHLY KPI · SOURCE · FORMULA · EVIDENCE','KPI THÁNG · NGUỒN · CÔNG THỨC · MINH CHỨNG')} title={teacherMode?bi(lang,'My monthly KPI','KPI tháng của tôi'):t(lang,'kpi')} text={teacherMode?bi(lang,'See exactly how your monthly KPI is built, how many learners dropped, how many approved issues were recorded and whether the data is complete.','Xem rõ KPI tháng được hình thành thế nào, có bao nhiêu HV rụng, bao nhiêu lỗi đã duyệt và dữ liệu đã đầy đủ đến đâu.'):bi(lang,'Record monthly learner base/dropouts, observation evidence and business contribution; Teacher 360 calculates HVR, KPI band and data completeness automatically.','Nhập số HV đầu kỳ/rụng, bằng chứng dự giờ và đóng góp kinh doanh; Teacher 360 tự tính HVR, xếp loại KPI và độ đầy đủ dữ liệu.')} actions={actions}/>
      {teacherMode&&own&&<div className="my-kpi-hero">
        <div className="my-kpi-score"><span>{bi(lang,'KPI BAND','XẾP LOẠI KPI')}</span><b>{own.band.code}</b><strong>{own.score??'—'}</strong><small>{lang==='vi'?own.band.vi:own.band.en} · {own.completeness}% {bi(lang,'data completeness','độ đầy đủ dữ liệu')}</small></div>
        <div className="my-kpi-metrics"><div><span>{bi(lang,'Observation','Dự giờ')}</span><b>{own.observation??'—'}</b></div><div><span>HVR</span><b>{own.hvr!=null?`${own.hvr}%`:'—'}</b><small>{own.learnerStart!=null?`${own.dropouts??0}/${own.learnerStart} ${bi(lang,'dropouts','HV rụng')}`:bi(lang,'Learner base missing','Thiếu số HV đầu kỳ')}</small></div><div><span>{bi(lang,'Approved issues','Lỗi đã duyệt')}</span><b>{own.issues.approved}</b><small>{own.issues.pending} {bi(lang,'pending','chờ duyệt')}</small></div><div><span>{bi(lang,'Penalty','Phạt')}</span><b className="penalty-off">{own.penaltyActive?Number(own.penaltyAmount).toLocaleString():bi(lang,'NOT APPLIED','CHƯA ÁP DỤNG')}</b></div></div>
      </div>}
      <div className="kpi-explain-strip"><div><b>{bi(lang,'HVR formula','Công thức HVR')}</b><span>{bi(lang,'(Learners at start − dropouts) ÷ learners at start × 100','(HV đầu kỳ − HV rụng) ÷ HV đầu kỳ × 100')}</span></div><div><b>{bi(lang,'KPI bands','Thang KPI')}</b><span>A ≥90 · B 80–89.9 · C 70–79.9 · D &lt;70</span></div><div><b>{bi(lang,'Issue rule today','Quy tắc lỗi hiện tại')}</b><span>{bi(lang,'Approved issues are visible but do not deduct KPI yet.','Lỗi đã duyệt được hiển thị nhưng hiện chưa trừ KPI.')}</span></div></div>
      {!teacherMode&&<div className="panel"><div className="table-wrap"><div className="table"><div className="tr th kpi-cols-v16"><span>{bi(lang,'Teacher','Giáo viên')}</span><span>{bi(lang,'Observation','Dự giờ')}</span><span>{bi(lang,'Learner base','HV đầu kỳ')}</span><span>{bi(lang,'Dropouts','HV rụng')}</span><span>HVR</span><span>{bi(lang,'Issues','Lỗi')}</span><span>KPI</span><span>{bi(lang,'Data','Dữ liệu')}</span></div>{rows.map(r=><div className="tr kpi-cols-v16" key={r.teacher.id}><span className="teacher-cell"><AvatarPic user={r.teacher}/><span><b>{r.teacher.full_name}</b><small>{r.teacher.home_centre_code||'—'} · {r.teacher.teacher_code||'—'}</small></span></span><span><b>{r.observation??'—'}</b></span><span><b>{r.learnerStart??'—'}</b></span><span><b>{r.dropouts??'—'}</b></span><span><b>{r.hvr!=null?`${r.hvr}%`:'—'}</b></span><span><b>{r.issues.approved}</b><small>{r.issues.pending} {bi(lang,'pending','chờ duyệt')}</small></span><span><KpiBandPill band={r.band} score={r.score} completeness={r.completeness} lang={lang}/></span><span><b>{r.completeness}%</b><small>{r.k?bi(lang,'Recorded','Đã chốt'):bi(lang,'Not recorded','Chưa chốt')}</small></span></div>)}{!rows.length&&<Empty lang={lang}/>}</div></div></div>}
      <div className="policy-off-banner"><b>{bi(lang,'No monetary penalty is being applied now.','Hiện tại chưa áp dụng phạt tiền.')}</b><span>{bi(lang,'The system keeps approved issue counts and a future penalty field so policy can be activated later with transparent historical evidence.','Hệ thống vẫn lưu số lỗi đã duyệt và trường mức phạt để sau này có thể kích hoạt chính sách với dữ liệu lịch sử minh bạch.')}</span></div>
    </>
  }
  ...
  function EvaluationFramework(){
    ...
    const kpiRows=[
      {
        no:'01',dimension:bi(lang,'Teaching quality','Chất lượng giảng dạy'),metric:bi(lang,'Official Observation score','Điểm Observation chính thức'),weight:'50%',
        source:bi(lang,'VMG Observation form / Performance Tracker','Phiếu Observation / Performance Tracker VMG'),
        how:bi(lang,'Monthly average of official observation scores. Observation itself is built from Preparation 10% + Methodology & Skills 45% + Classroom Management 45%.','Lấy trung bình các điểm Observation chính thức trong tháng. Bản thân Observation được cấu thành từ Preparation 10% + Methodology & Skills 45% + Classroom Management 45%.'),
        evidence:bi(lang,'Observation record, criterion ratings, observer feedback, strengths and development areas.','Phiếu dự giờ, điểm từng tiêu chí, nhận xét người dự giờ, điểm mạnh và điểm cần phát triển.'),
        interpretation:bi(lang,'>90 Excellent · 81–90 Very good · 70–80 Good · 60–69 Satisfactory · <60 Development / Training needed','>90 Xuất sắc · 81–90 Rất tốt · 70–80 Tốt · 60–69 Đạt · <60 Cần cải thiện / đào tạo')
      },
      {
        no:'02',dimension:bi(lang,'Learner impact','Tác động tới học viên'),metric:'HVR · Learner Retention',weight:'30%',
        source:bi(lang,'Learners at start + learner dropouts recorded for the month','Số HV đầu kỳ + số HV rụng được ghi nhận trong tháng'),
        how:bi(lang,'HVR = (Learners at start − dropouts) ÷ learners at start × 100. The system calculates the percentage automatically.','HVR = (HV đầu kỳ − HV rụng) ÷ HV đầu kỳ × 100. Hệ thống tự tính tỷ lệ.'),
        evidence:bi(lang,'Monthly learner base and dropout count. A missing learner base is shown as missing data, not silently scored as zero.','Số HV đầu kỳ và số HV rụng theo tháng. Nếu thiếu số HV đầu kỳ, hệ thống báo thiếu dữ liệu chứ không tự tính thành 0.'),
        interpretation:bi(lang,'Numeric retention evidence. Target threshold remains configurable until the official policy is approved.','Minh chứng duy trì học viên dạng số. Ngưỡng target vẫn có thể cấu hình cho tới khi chính sách chính thức được phê duyệt.')
      },
      {
        no:'03',dimension:bi(lang,'Business contribution','Đóng góp kinh doanh'),metric:bi(lang,'Revenue contribution index','Chỉ số doanh thu đóng góp'),weight:'20%',
        source:bi(lang,'Monthly recorded revenue contribution','Doanh thu đóng góp được ghi nhận theo tháng'),
        how:bi(lang,`Index = revenue ÷ ${KPI_REVENUE_TARGET.toLocaleString()} × 100, capped at 100.`,`Chỉ số = doanh thu ÷ ${KPI_REVENUE_TARGET.toLocaleString()} × 100, tối đa 100.`),
        evidence:bi(lang,'Monthly recorded contribution; target is configurable.','Dữ liệu đóng góp theo tháng; mốc target có thể cấu hình.'),
        interpretation:bi(lang,'Operating KPI component V1; the target is not treated as a permanent policy constant.','Thành phần KPI vận hành V1; mốc target chưa được coi là hằng số chính sách lâu dài.')
      },
      {
        no:'04',dimension:bi(lang,'Professional reliability','Tính chuyên nghiệp & tuân thủ'),metric:bi(lang,'Approved issue count','Số lỗi / sự vụ đã duyệt'),weight:'0%',
        source:bi(lang,'Director-approved cases with evidence','Sự vụ có evidence và đã được GĐTT phê duyệt'),
        how:bi(lang,'Approved, pending and critical cases are counted separately by month.','Lỗi đã duyệt, đang chờ duyệt và nghiêm trọng được đếm riêng theo tháng.'),
        evidence:bi(lang,'Case record, category, severity, evidence, approval and follow-up history.','Hồ sơ sự vụ, nhóm lỗi, mức độ, evidence, phê duyệt và lịch sử follow-up.'),
        interpretation:bi(lang,'VISIBLE FOR TRANSPARENCY. No KPI deduction or monetary penalty is applied at this stage.','HIỂN THỊ ĐỂ MINH BẠCH. Hiện chưa trừ KPI và chưa áp dụng phạt tiền.')
      }
    ]
    const competencies=[
      {
        domain:bi(lang,'Preparation & readiness','Chuẩn bị & sẵn sàng'),weight:'10% OBS',
        evidence:bi(lang,'Lesson objectives, materials, readiness, class setup and programme alignment.','Mục tiêu bài học, học liệu, sự sẵn sàng, setup lớp và bám chương trình.'),
        l1:bi(lang,'Often unprepared; objectives/materials are unclear or incomplete.','Thường chưa sẵn sàng; mục tiêu/học liệu chưa rõ hoặc thiếu.'),
        l2:bi(lang,'Basic preparation is present but consistency or anticipation of learner needs is limited.','Có chuẩn bị cơ bản nhưng tính ổn định hoặc dự đoán nhu cầu HV còn hạn chế.'),
        l3:bi(lang,'Consistently prepared, organised and aligned to intended outcomes.','Chuẩn bị ổn định, có tổ chức và bám đúng đầu ra dự kiến.'),
        l4:bi(lang,'Highly purposeful preparation; anticipates learner needs and adapts resources intelligently.','Chuẩn bị có chủ đích cao; dự đoán nhu cầu HV và điều chỉnh học liệu linh hoạt.')
      },
      {
        domain:bi(lang,'Teaching methodology & skills','Phương pháp & kỹ năng giảng dạy'),weight:'45% OBS',
        evidence:bi(lang,'Clarity of instruction, modelling, ICQs/CCQs, practice design, feedback, assessment and language use.','Độ rõ của hướng dẫn, modelling, ICQ/CCQ, thiết kế luyện tập, feedback, assessment và sử dụng ngôn ngữ.'),
        l1:bi(lang,'Teacher-led delivery dominates; checking and practice do not reliably support outcomes.','Giờ học còn nặng teacher-led; kiểm tra và luyện tập chưa hỗ trợ đầu ra ổn định.'),
        l2:bi(lang,'Uses core techniques but execution varies and feedback may not lead to a stronger next attempt.','Có dùng kỹ thuật cốt lõi nhưng thực thi chưa đều; feedback chưa luôn dẫn tới lần làm tốt hơn.'),
        l3:bi(lang,'Uses appropriate methodology, checks understanding and creates meaningful learner practice.','Dùng phương pháp phù hợp, kiểm tra hiểu và tạo đủ thực hành có ý nghĩa cho HV.'),
        l4:bi(lang,'Selects and adapts methodology expertly; evidence shows strong learning impact and responsive feedback.','Lựa chọn/điều chỉnh phương pháp rất tốt; evidence cho thấy tác động học tập và feedback đáp ứng cao.')
      },
      {
        domain:bi(lang,'Classroom management','Quản lý lớp học'),weight:'45% OBS',
        evidence:bi(lang,'Participation, routines, pace, transitions, learner engagement, differentiation and classroom climate.','Mức tham gia, routines, pace, chuyển hoạt động, engagement, differentiation và không khí lớp.'),
        l1:bi(lang,'Participation, pacing or routines frequently limit learning.','Mức tham gia, tốc độ hoặc routines thường cản trở việc học.'),
        l2:bi(lang,'Basic control is present but participation, transitions or differentiation are inconsistent.','Có kiểm soát cơ bản nhưng participation, chuyển hoạt động hoặc differentiation chưa ổn định.'),
        l3:bi(lang,'Maintains clear routines, productive pace and broad learner participation.','Duy trì routines rõ, pace hiệu quả và participation rộng.'),
        l4:bi(lang,'Creates a highly responsive, inclusive and productive environment across learner profiles.','Tạo môi trường học đáp ứng cao, hòa nhập và hiệu quả với nhiều nhóm HV.')
      },
      {
        domain:bi(lang,'Learner impact & retention','Tác động & duy trì học viên'),weight:bi(lang,'KPI evidence','Minh chứng KPI'),
        evidence:bi(lang,'HVR, learner base, dropouts and learner/service evidence.','HVR, số HV đầu kỳ, số HV rụng và evidence học viên/dịch vụ.'),
        l1:bi(lang,'Retention evidence is weak, missing or shows repeated unresolved learner loss signals.','Evidence duy trì yếu/thiếu hoặc xuất hiện lặp lại tín hiệu HV rụng chưa được xử lý.'),
        l2:bi(lang,'Retention is monitored but follow-up and root-cause response are still developing.','Có theo dõi duy trì nhưng follow-up và xử lý nguyên nhân gốc còn đang phát triển.'),
        l3:bi(lang,'Retention is stable with timely follow-up and clear learner-care actions.','Duy trì ổn định, follow-up kịp thời và có hành động chăm sóc HV rõ.'),
        l4:bi(lang,'Shows strong learner impact and proactively prevents avoidable dropout patterns.','Cho thấy tác động mạnh tới HV và chủ động ngăn các pattern rụng có thể phòng tránh.')
      },
      {
        domain:bi(lang,'Professional conduct & growth','Tác phong & phát triển nghề nghiệp'),weight:bi(lang,'Development evidence','Minh chứng phát triển'),
        evidence:bi(lang,'Approved cases, punctuality/conduct evidence, coaching, training, reflection and follow-through.','Lỗi đã duyệt, evidence đúng giờ/tác phong, coaching, training, reflection và follow-through.'),
        l1:bi(lang,'Repeated reliability concerns or limited response to feedback.','Có vấn đề lặp lại về độ tin cậy hoặc phản hồi với feedback còn hạn chế.'),
        l2:bi(lang,'Generally cooperative but improvement actions are not yet consistent.','Nhìn chung hợp tác nhưng hành động cải thiện chưa ổn định.'),
        l3:bi(lang,'Reliable, responsive to feedback and follows agreed development actions.','Đáng tin cậy, phản hồi tốt với feedback và thực hiện action phát triển đã thống nhất.'),
        l4:bi(lang,'Models professional standards, owns development and contributes positively to peers/team.','Làm gương về chuẩn nghề nghiệp, chủ động phát triển và đóng góp tích cực cho đồng nghiệp/đội ngũ.')
      }
    ]
    const issueLevels=[
      {code:'01',level:bi(lang,'Routine monitoring','Theo dõi bình thường'),threshold:'0–1 / week',action:bi(lang,'Normal monitoring; no automatic negative conclusion.','Theo dõi định kỳ; không tự động kết luận tiêu cực.'),tone:'stable'},
      {code:'02',level:bi(lang,'Watch','Theo dõi'),threshold:'2 / week or 3 / month',action:bi(lang,'Coaching + targeted observation / follow-up.','Coaching + dự giờ/follow-up có mục tiêu.'),tone:'watch'},
      {code:'03',level:bi(lang,'High risk','Rủi ro cao'),threshold:'≥3 / week or ≥4 / month',action:bi(lang,'Formal review + retraining + cautious class allocation.','Đánh giá chính thức + đào tạo lại + thận trọng khi phân lớp.'),tone:'danger'},
      {code:'04',level:bi(lang,'Critical','Nghiêm trọng'),threshold:bi(lang,'Any critical breach','Bất kỳ lỗi critical nào'),action:bi(lang,'Immediate escalation and formal review.','Nâng mức xử lý ngay và đánh giá chính thức.'),tone:'critical'}
    ]
    const process=[
      ['01',bi(lang,'Capture evidence','Ghi nhận dữ liệu'),bi(lang,'Observation, learner base/dropouts, approved cases, training/coaching and other evidence are recorded in Teacher 360.','Observation, HV đầu kỳ/rụng, sự vụ đã duyệt, training/coaching và các evidence khác được ghi nhận trên Teacher 360.')],
      ['02',bi(lang,'Calculate monthly KPI','Tính KPI tháng'),bi(lang,'Teacher 360 calculates HVR, weighted KPI and data completeness. Missing data stays visible as missing.','Teacher 360 tự tính HVR, KPI có trọng số và độ đầy đủ dữ liệu. Dữ liệu thiếu vẫn được hiển thị là thiếu.')],
      ['03',bi(lang,'Explain the result','Giải thích kết quả'),bi(lang,'Teachers can see the component scores, rubric, evidence history and issue count — not just one final number.','GV thấy từng thành phần, rubric, lịch sử evidence và số lỗi — không chỉ một con số cuối cùng.')],
      ['04',bi(lang,'Develop & follow up','Phát triển & follow-up'),bi(lang,'Coaching, training, re-observation and agreed development actions are linked back to the same portfolio.','Coaching, training, re-observation và action phát triển được nối lại vào cùng hồ sơ.')],
    ]
    return <>
      <PageIntro
        kicker={bi(lang,'TRANSPARENT EVALUATION · EVIDENCE · DEVELOPMENT','ĐÁNH GIÁ MINH BẠCH · MINH CHỨNG · PHÁT TRIỂN')}
        title={teacherMode?bi(lang,'How I am evaluated','Tôi được đánh giá như thế nào?'):bi(lang,'Teacher Evaluation Framework','Khung đánh giá giáo viên')}
        text={teacherMode?bi(lang,'This page explains exactly what is measured, where each number comes from, what the rating bands mean, and which evidence is used for your development.','Trang này giải thích rõ bạn được đo ở đâu, từng con số lấy từ nguồn nào, thang điểm có ý nghĩa gì và evidence nào được dùng cho hành trình phát triển của bạn.'):bi(lang,'One shared framework for KPI, observation, competency, issue-risk and development evidence across Teacher 360.','Một khung chung để hiểu KPI, observation, năng lực, ngưỡng sự vụ và evidence phát triển trên Teacher 360.')}
        actions={<div className="page-action-row"><button className="btn secondary" onClick={()=>setView('kpi')}>% {bi(lang,'Open monthly KPI','Mở KPI tháng')}</button><button className="btn soft" onClick={()=>setView('observations')}>✓ {bi(lang,'Open observation history','Mở lịch sử dự giờ')}</button></div>}
      />

      <section className="evaluation-principles">
        <article><i>01</i><div><b>{bi(lang,'No hidden score','Không có điểm ẩn')}</b><p>{bi(lang,'Every scored KPI component has a source, formula and visible weight.','Mỗi thành phần KPI có nguồn, công thức và trọng số hiển thị rõ.')}</p></div></article>
        <article><i>02</i><div><b>{bi(lang,'Missing ≠ zero','Thiếu dữ liệu ≠ 0 điểm')}</b><p>{bi(lang,'Data completeness is shown separately so missing evidence is not silently treated as poor performance.','Độ đầy đủ dữ liệu được hiển thị riêng để evidence thiếu không bị âm thầm hiểu thành hiệu suất kém.')}</p></div></article>
        <article><i>03</i><div><b>{bi(lang,'Evidence before conclusion','Evidence trước kết luận')}</b><p>{bi(lang,'Observation feedback, learner data and approved case evidence stay traceable in the portfolio.','Feedback dự giờ, dữ liệu HV và evidence sự vụ đã duyệt đều truy vết được trong hồ sơ.')}</p></div></article>
        <article><i>04</i><div><b>{bi(lang,'Development, not only ranking','Phát triển, không chỉ xếp hạng')}</b><p>{bi(lang,'Competency evidence informs coaching, training, re-observation and career discussions.','Evidence năng lực được dùng để định hướng coaching, training, re-observation và trao đổi lộ trình.')}</p></div></article>
      </section>

      <section className="framework-section">
        <div className="framework-section-head"><div><span>{bi(lang,'MATRIX 01 · MONTHLY PERFORMANCE','MA TRẬN 01 · HIỆU SUẤT THÁNG')}</span><h3>{bi(lang,'How the monthly KPI is built','KPI tháng được hình thành như thế nào')}</h3><p>{bi(lang,'The KPI score and the professional competency matrix are related but not identical: KPI measures monthly performance; competency evidence supports development and upgrade decisions.','Điểm KPI và ma trận năng lực có liên quan nhưng không đồng nhất: KPI đo hiệu suất theo tháng; ma trận năng lực hỗ trợ phát triển và quyết định nâng cấp độ.')}</p></div><span className="framework-status">{bi(lang,'Operating framework V1','Khung vận hành V1')}</span></div>
        <div className="framework-table-wrap">
          <div className="framework-table kpi-framework-table">
            <div className="framework-tr framework-th"><span>#</span><span>{bi(lang,'Dimension / metric','Khía cạnh / chỉ số')}</span><span>{bi(lang,'Weight','Trọng số')}</span><span>{bi(lang,'Data source','Nguồn dữ liệu')}</span><span>{bi(lang,'Calculation','Cách tính')}</span><span>{bi(lang,'Evidence & interpretation','Evidence & cách hiểu')}</span></div>
            {kpiRows.map(x=><div className="framework-tr" key={x.no}><span className="matrix-index">{x.no}</span><span><b>{x.dimension}</b><small>{x.metric}</small></span><span><strong>{x.weight}</strong></span><span><b>{x.source}</b></span><span><p>{x.how}</p></span><span><b>{x.interpretation}</b><small>{x.evidence}</small></span></div>)}
          </div>
        </div>
        <div className="framework-band-row"><span><b>A · ≥90</b>{bi(lang,'High performance','Hiệu suất cao')}</span><span><b>B · 80–89.9</b>{bi(lang,'Strong / on track','Tốt / đúng hướng')}</span><span><b>C · 70–79.9</b>{bi(lang,'Needs targeted development','Cần phát triển có mục tiêu')}</span><span><b>D · &lt;70</b>{bi(lang,'Development priority','Ưu tiên cải thiện')}</span></div>
      </section>

      <section className="framework-section">
        <div className="framework-section-head"><div><span>{bi(lang,'MATRIX 02 · PROFESSIONAL COMPETENCY','MA TRẬN 02 · NĂNG LỰC NGHỀ NGHIỆP')}</span><h3>{bi(lang,'What “good teaching” looks like in observable behaviour','Một giáo viên tốt được thể hiện qua hành vi quan sát được như thế nào')}</h3><p>{bi(lang,'Ratings 1–4 are used as a development language. The three Observation domains retain the official VMG weighting: Preparation 10%, Methodology & Skills 45%, Classroom Management 45%.','Rating 1–4 được dùng như ngôn ngữ phát triển. Ba nhóm Observation vẫn giữ trọng số VMG: Preparation 10%, Methodology & Skills 45%, Classroom Management 45%.')}</p></div><span className="framework-status soft">{bi(lang,'Development lens','Khung phát triển')}</span></div>
        <div className="framework-table-wrap">
          <div className="framework-table competency-framework-table">
            <div className="framework-tr framework-th"><span>{bi(lang,'Competency domain','Nhóm năng lực')}</span><span>{bi(lang,'Evidence','Evidence')}</span><span>1 · {bi(lang,'Needs support','Cần hỗ trợ')}</span><span>2 · {bi(lang,'Developing','Đang phát triển')}</span><span>3 · {bi(lang,'Meets standard','Đạt chuẩn')}</span><span>4 · {bi(lang,'Strong / role model','Mạnh / hình mẫu')}</span></div>
            {competencies.map(x=><div className="framework-tr" key={x.domain}><span><b>{x.domain}</b><small>{x.weight}</small></span><span><p>{x.evidence}</p></span><span><p>{x.l1}</p></span><span><p>{x.l2}</p></span><span><p>{x.l3}</p></span><span><p>{x.l4}</p></span></div>)}
          </div>
        </div>
        <div className="framework-note"><b>{bi(lang,'Important distinction','Phân biệt quan trọng')}</b><p>{bi(lang,'Competency ratings are evidence for feedback, coaching, training and career/level discussions. They are not automatically converted into a monetary penalty or hidden KPI deduction.','Rating năng lực là evidence cho feedback, coaching, training và trao đổi level/lộ trình. Chúng không tự động chuyển thành phạt tiền hoặc khoản trừ KPI ẩn.')}</p></div>
      </section>
      ...
      <section className="framework-section">
        <div className="framework-section-head"><div><span>{bi(lang,'THE MONTHLY CYCLE','CHU TRÌNH HÀNG THÁNG')}</span><h3>{bi(lang,'From raw evidence to a development conversation','Từ dữ liệu thô đến trao đổi phát triển')}</h3></div></div>
        <div className="framework-process">{process.map(x=><article key={x[0]}><i>{x[0]}</i><div><b>{x[1]}</b><p>{x[2]}</p></div></article>)}</div>
      </section>

      <section className="framework-visibility">
        <div><span>{bi(lang,'WHAT THE TEACHER CAN SEE','GV NHÌN THẤY GÌ')}</span><h3>{bi(lang,'Your evaluation is not a black box.','Đánh giá của bạn không phải “hộp đen”.')}</h3></div>
        <div className="visibility-grid">
          <span>✓ {bi(lang,'Monthly KPI score + band','Điểm KPI tháng + xếp loại')}</span>
          <span>✓ {bi(lang,'Component metrics + data completeness','Từng chỉ số thành phần + độ đầy đủ dữ liệu')}</span>
          <span>✓ {bi(lang,'Observation history + feedback','Lịch sử Observation + feedback')}</span>
          <span>✓ {bi(lang,'Learners at start + dropouts + HVR','HV đầu kỳ + HV rụng + HVR')}</span>
          <span>✓ {bi(lang,'Approved / pending / critical issue counts','Số lỗi đã duyệt / chờ duyệt / critical')}</span>
          <span>✓ {bi(lang,'Evidence, coaching, training and development actions','Evidence, coaching, training và action phát triển')}</span>
        </div>
      </section>
    </>
  }
  ...
  function Handbook(){
    const standards=lang==='vi'?[
      {icon:'01',title:'Đúng giờ & sẵn sàng',points:['Có mặt ít nhất 5 phút trước giờ học và bảo đảm lớp sẵn sàng đúng giờ.','Kiểm tra học liệu, thiết bị, công nghệ và có phương án dự phòng trước khi bắt đầu.','Nếu có nguy cơ trễ hoặc vắng, báo ngay theo kênh vận hành của trung tâm.']},
      {icon:'02',title:'Chất lượng giờ dạy',points:['Dạy theo cấu trúc bài học VMG đã được phê duyệt và làm rõ mục tiêu học tập.','Sử dụng ICQs, hoạt động có mục đích và tối đa hóa thời gian học viên thực hành.','Duy trì độ chính xác ngôn ngữ và điều chỉnh cách dạy phù hợp trình độ học viên.']},
      {icon:'03',title:'Quản lý lớp & chăm sóc học viên',points:['Bảo đảm mọi học viên có cơ hội tham gia và được đối xử tôn trọng.','Duy trì lớp học an toàn, hòa nhập, có nề nếp và phản hồi câu hỏi của học viên phù hợp.','Các vấn đề an toàn, đạo đức hoặc safeguarding phải được báo cáo ngay, không tự xử lý ngoài quy trình.']},
      {icon:'04',title:'Lịch, giao tiếp & phối hợp',points:['Chủ động cập nhật lịch rảnh và phản hồi các yêu cầu điều phối đúng thời hạn.','Yêu cầu đổi lịch cần được gửi sớm theo quy định vận hành; tránh thay đổi sát giờ nếu không phải tình huống bất khả kháng.','Trao đổi chuyên nghiệp với CMO/GĐTT, AS, PTNS, CSR và các bộ phận liên quan.']},
      {icon:'05',title:'Hồ sơ & minh chứng',points:['Tự tải bằng cấp, chứng chỉ và hồ sơ cá nhân lên Teacher 360 để PTNS kiểm tra.','Bổ sung hồ sơ đúng ngày đã cam kết khi có yêu cầu.','Không chỉnh sửa, xóa hoặc làm sai lệch bằng chứng chuyên môn và lịch sử hoạt động.']},
      {icon:'06',title:'Phản hồi & phát triển chuyên môn',points:['Tham gia dự giờ, coaching, retraining và các hoạt động phát triển được phân công.','Đọc phản hồi, thống nhất hành động phát triển và theo dõi tiến độ đến lần đánh giá tiếp theo.','Chủ động yêu cầu hỗ trợ khi có khó khăn về phương pháp, lớp học, học liệu hoặc vận hành.']}
    ]:[
      {icon:'01',title:'Punctuality & readiness',points:['Arrive at least five minutes before class and have the room ready on time.','Check materials, equipment and technology in advance, with a workable backup.','Flag a likely late arrival or absence immediately through the centre operating channel.']},
      {icon:'02',title:'Teaching quality',points:['Follow the approved VMG lesson structure and make learning outcomes clear.','Use ICQs, purposeful activities and maximise meaningful learner practice.','Maintain accurate English and adapt delivery to learner level and need.']},
      {icon:'03',title:'Classroom & learner care',points:['Give all learners a fair opportunity to participate and be treated with dignity.','Maintain a safe, inclusive and supportive classroom with clear routines.','Safeguarding or ethical concerns must be escalated immediately through the formal process.']},
      {icon:'04',title:'Schedule & communication',points:['Keep availability current and respond to coordination requests on time.','Request schedule changes early under centre operating rules and avoid last-minute changes unless unavoidable.','Communicate professionally with centre leaders, AS, HR, CSR and relevant teams.']},
      {icon:'05',title:'Documents & evidence',points:['Upload qualifications and personal employment documents directly to Teacher 360 for HR review.','Complete missing documents by the date you have committed to.','Never alter, remove or misrepresent professional evidence or activity history.']},
      {icon:'06',title:'Feedback & professional growth',points:['Participate in observations, coaching, retraining and assigned development activities.','Review feedback, agree the next action and follow through before the next check.','Ask for support early when methodology, classroom, resources or operations become difficult.']}
    ]
    const process=lang==='vi'?[
      ['1','Ghi nhận bằng chứng','Sự vụ được ghi nhận theo nhóm, mức độ và bằng chứng cụ thể.'],
      ['2','Xem xét công bằng','Quản lý/đơn vị chuyên môn kiểm tra ngữ cảnh trước khi kết luận.'],
      ['3','Thông tin & hỗ trợ GV','Kết quả đã duyệt được thông tin cho giáo viên; hành động hỗ trợ, coaching hoặc retraining được gắn rõ.'],
      ['4','Theo dõi đến khi đóng','Có người phụ trách, hạn xử lý và minh chứng hoàn tất trước khi đóng sự vụ.']
    ]:[
      ['1','Record the evidence','The case is logged with a clear category, priority and supporting evidence.'],
      ['2','Review fairly','Centre/academic leaders review context before reaching a conclusion.'],
      ['3','Inform & support the teacher','Approved outcomes are shared with the teacher, with clear support, coaching or retraining actions.'],
      ['4','Follow through to closure','Every case keeps an owner, due date and completion evidence before closure.']
    ]
    return <><PageIntro kicker={bi(lang,'TEACHER STANDARDS · FAIRNESS · PROFESSIONAL TRUST','QUY ĐỊNH GV · CÔNG BẰNG · TRÁCH NHIỆM NGHỀ NGHIỆP')} title={t(lang,'handbook')} text={bi(lang,'A practical guide to the behaviours, evidence and support expected in day-to-day teaching at VMG. The purpose is clarity and consistency — not surprise rules after an issue happens.','Hướng dẫn thực tế về tác phong, bằng chứng và cách phối hợp trong công việc giảng dạy hằng ngày tại VMG. Mục tiêu là rõ ràng và nhất quán — không để đến khi có sự vụ mới xuất hiện quy định.')}/>
      <section className="handbook-hero"><div><span>{bi(lang,'OUR PROFESSIONAL PROMISE','CAM KẾT NGHỀ NGHIỆP')}</span><h2>{bi(lang,'Teach with care. Work with evidence. Grow through feedback.','Dạy bằng sự tận tâm. Làm việc bằng minh chứng. Phát triển qua phản hồi.')}</h2><p>{bi(lang,'These standards sit alongside approved centre policies and employment regulations. Where a formal policy is stricter, the formal policy applies.','Các nguyên tắc này được áp dụng cùng quy định chính thức của trung tâm và chính sách nhân sự. Nếu văn bản chính thức có yêu cầu chặt hơn, văn bản chính thức được ưu tiên.')}</p></div><div className="handbook-mark"><b>360°</b><span>{bi(lang,'Professional responsibility','Trách nhiệm nghề nghiệp')}</span></div></section>
      <div className="handbook-grid">{standards.map(x=><article className="handbook-card" key={x.icon}><span className="handbook-no">{x.icon}</span><h3>{x.title}</h3><ul>{x.points.map(p=><li key={p}>{p}</li>)}</ul></article>)}</div>
      <section className="panel handbook-process"><div className="panel-head"><div><span>{bi(lang,'WHEN SOMETHING GOES WRONG','KHI CÓ SỰ VỤ')}</span><h3>{bi(lang,'How Teacher 360 handles issues fairly','Cách Teacher 360 xử lý sự vụ công bằng')}</h3></div></div><div className="handbook-steps">{process.map(x=><article key={x[0]}><i>{x[0]}</i><div><b>{x[1]}</b><p>{x[2]}</p></div></article>)}</div><div className="friendly-note"><b>{bi(lang,'Need help before it becomes a case?','Cần hỗ trợ trước khi thành sự vụ?')}</b><p>{bi(lang,'Use Help & Support early. Asking for support is part of professional practice, not a negative signal.','Hãy dùng mục Hỗ trợ sớm. Chủ động xin hỗ trợ là một phần của thực hành nghề nghiệp, không phải tín hiệu tiêu cực.')}</p><button className="btn secondary" onClick={()=>setView('support')}>{bi(lang,'Open Help & Support','Mở mục Hỗ trợ')} →</button></div></section>
    </>
  }
  ...
  function renderModal(){
    ...
    if(modal.type==='kpi')return <KpiForm teachers={data.teachers} lang={lang} defaultMonth={modal.month} onSave={saveKpi}/>
    ...
    if(modal.type==='edit-user')return <UserForm lang={lang} initialUser={modal.user} onSave={updateUserAccount}/>
  ...
  async function syncGoogleCalendar(sourceType,sourceId){
    try{
      const res=await fetch('/api/calendar/sync',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sourceType,sourceId})})
      const out=await res.json().catch(()=>({}))
      return {ok:res.ok,...out}
    }catch(error){return {ok:false,error:error?.message||'Calendar sync unavailable'}}
  }
  async function saveEvent(form){const payload={...form,ends_at:form.ends_at||null,booked_by:profile.id,booked_by_name:profile.full_name};const {data:row,error}=await supabase.from('teacher_events').insert(payload).select('id').single();if(error)flash(error.message);else{setModal(null);const cal=await syncGoogleCalendar('teacher_event',row.id);flash(cal.ok?bi(lang,'Booked and added to the teacher’s Google Calendar.','Đã đặt lịch và tự động thêm vào Google Calendar của giáo viên.'):bi(lang,'Booking saved. Google Calendar is pending — you can retry from the calendar page.','Đã lưu lịch. Google Calendar đang chờ đồng bộ — có thể thử lại tại trang Lịch.'));afterAction()}}
  async function saveKpi(form){
    const learnerStart=Math.max(0,Number(form.learner_start_count||0)),dropouts=Math.max(0,Math.min(learnerStart,Number(form.learner_dropout_count||0)))
    const hvr=retentionPct(learnerStart,dropouts)
    const obsInput=form.observation_score===''||form.observation_score==null?null:Number(form.observation_score)
    const obs=obsInput!=null?obsInput:monthlyObservation(form.teacher_id,String(form.period_start).slice(0,7))
    const rev=form.revenue_contribution===''||form.revenue_contribution==null?null:Number(form.revenue_contribution)
    const revIndex=rev!=null?Math.min(100,Math.max(0,Number((rev/KPI_REVENUE_TARGET*100).toFixed(1)))):null
    const weighted=weightedKpiComposite({observation:obs,hvr,revenueIndex,weightObservation:50,weightHvr:30,weightRevenue:20})
    const issues=data.incidents.filter(x=>x.teacher_id===form.teacher_id&&new Date(x.created_at)>=new Date(`${form.period_start}T00:00:00`)&&new Date(x.created_at)<=new Date(`${form.period_end}T23:59:59`))
    const approved=issues.filter(x=>!!x.director_approved_at),pending=issues.filter(x=>!x.director_approved_at),critical=approved.filter(x=>x.severity==='critical')
    const payload={teacher_id:form.teacher_id,period_start:form.period_start,period_end:form.period_end,observation_score:obs,hvr_pct:hvr,learner_start_count:learnerStart||null,learner_dropout_count:dropouts,incident_count:approved.length,pending_incident_count:pending.length,critical_incident_count:critical.length,revenue_contribution:rev,composite_score:weighted.score,data_completeness_pct:weighted.completeness,kpi_band:kpiBand(weighted.score).code,penalty_policy_active:false,penalty_amount:0,notes:form.notes||null,recorded_by:profile.id}
    const {error}=await supabase.from('teacher_kpi_snapshots').upsert(payload,{onConflict:'teacher_id,period_start,period_end'});if(error)flash(error.message);else{setModal(null);flash(bi(lang,'Monthly KPI saved. HVR, issue counts, band and data completeness were calculated automatically.','Đã lưu KPI tháng. HVR, số lỗi, xếp loại và độ đầy đủ dữ liệu đã được tính tự động.'));afterAction()}}
  ...
  async function bookTraining(trainingId){const {data:row,error}=await supabase.from('training_registrations').insert({training_id:trainingId,user_id:profile.id,status:'booked'}).select('id').single();if(error)flash(error.message);else{const cal=await syncGoogleCalendar('training_registration',row.id);flash(cal.ok?bi(lang,'Booking confirmed and added to your Google Calendar.','Đã xác nhận tham gia và tự động thêm vào Google Calendar của bạn.'):bi(lang,'Booking confirmed. Google Calendar is pending; the in-app 24-hour reminder is still active.','Đã xác nhận tham gia. Google Calendar đang chờ đồng bộ; nhắc lịch T−24h trong hệ thống vẫn hoạt động.'));afterAction()}}
  ...
  function monthBounds(month=nowDate().slice(0,7)){
    const [y,m]=String(month).split('-').map(Number)
    const start=new Date(y,m-1,1),end=new Date(y,m,0,23,59,59,999)
    return {start,end,startText:`${y}-${String(m).padStart(2,'0')}-01`,endText:`${y}-${String(m).padStart(2,'0')}-${String(end.getDate()).padStart(2,'0')}`}
  }
  function inMonth(value,month){if(!value)return false;const d=new Date(value),{start,end}=monthBounds(month);return d>=start&&d<=end}
  function kpiForMonth(id,month){return data.kpis.find(x=>x.teacher_id===id&&String(x.period_start||'').slice(0,7)===month)}
  function monthlyObservation(id,month){const vals=data.observations.filter(x=>x.teacher_id===id&&x.final_score!==null&&inMonth(x.observed_at,month)).map(x=>Number(x.final_score)).filter(Number.isFinite);return vals.length?Number((vals.reduce((a,b)=>a+b,0)/vals.length).toFixed(1)):null}
  function monthlyIssueSummary(id,month){
    const rows=data.incidents.filter(x=>x.teacher_id===id&&inMonth(x.created_at,month))
    const approved=rows.filter(x=>!!x.director_approved_at),pending=rows.filter(x=>!x.director_approved_at)
    return {total:rows.length,approved:approved.length,pending:pending.length,critical:approved.filter(x=>x.severity==='critical').length,rows}
  }
  function monthlyKpiIntel(teacher,month=nowDate().slice(0,7)){
    const k=kpiForMonth(teacher.id,month),issues=monthlyIssueSummary(teacher.id,month)
    const observation=k?.observation_score!=null?Number(k.observation_score):monthlyObservation(teacher.id,month)
    const learnerStart=k?.learner_start_count!=null?Number(k.learner_start_count):null,dropouts=k?.learner_dropout_count!=null?Number(k.learner_dropout_count):null
    const computedHvr=retentionPct(learnerStart,dropouts)
    const hvr=computedHvr!=null?computedHvr:(k?.hvr_pct!=null?Number(k.hvr_pct):null)
    const revenue=k?.revenue_contribution!=null?Number(k.revenue_contribution):null
    const revenueIndex=revenue!=null?Math.min(100,Math.max(0,Number((revenue/KPI_REVENUE_TARGET*100).toFixed(1)))):null
    const weighted=weightedKpiComposite({observation,hvr,revenueIndex,weightObservation:Number(k?.weight_observation??50),weightHvr:Number(k?.weight_hvr??30),weightRevenue:Number(k?.weight_revenue??20)})
    const score=k?.composite_score!=null?Number(k.composite_score):weighted.score
    const completeness=k?.data_completeness_pct!=null?Number(k.data_completeness_pct):weighted.completeness
    const band=kpiBand(score)
    return {k,month,observation,learnerStart,dropouts,learnerEnd:learnerStart?Math.max(0,learnerStart-dropouts):null,hvr,revenue,revenueIndex,score,completeness,band,issues:{...issues,approved:k?.incident_count!=null?Number(k.incident_count):issues.approved,pending:k?.pending_incident_count!=null?Number(k.pending_incident_count):issues.pending,critical:k?.critical_incident_count!=null?Number(k.critical_incident_count):issues.critical},penaltyActive:Boolean(k?.penalty_policy_active),penaltyAmount:Number(k?.penalty_amount||0)}
  }
  async function exportMonthlyKpi(month,teachersToExport=data.teachers){
    const XLSX=await import('xlsx')
    const rows=teachersToExport.map(teacher=>{const x=monthlyKpiIntel(teacher,month);return {
      [bi(lang,'Month','Tháng')]:month,
      [bi(lang,'Teacher','Giáo viên')]:teacher.full_name,
      [bi(lang,'Teacher code','Mã GV')]:teacher.teacher_code||'',
      [bi(lang,'Centre','Trung tâm')]:teacher.home_centre_code||'',
      [bi(lang,'Region','Khu vực')]:teacher.region_no||'',
      [bi(lang,'Observation score','Điểm dự giờ')]:x.observation??'',
      [bi(lang,'Learners at start','HV đầu kỳ')]:x.learnerStart??'',
      [bi(lang,'Dropouts','HV rụng')]:x.dropouts??'',
      [bi(lang,'Learners retained','HV còn lại')]:x.learnerEnd??'',
      'HVR %':x.hvr??'',
      [bi(lang,'Revenue contribution','Doanh thu đóng góp')]:x.revenue??'',
      [bi(lang,'Approved issues','Lỗi đã duyệt')]:x.issues.approved,
      [bi(lang,'Pending issues','Lỗi chờ duyệt')]:x.issues.pending,
      [bi(lang,'Critical issues','Lỗi nghiêm trọng')]:x.issues.critical,
      [bi(lang,'KPI score','Điểm KPI')]:x.score??'',
      [bi(lang,'KPI band','Xếp loại KPI')]:`${x.band.code} · ${lang==='vi'?x.band.vi:x.band.en}`,
      [bi(lang,'Data completeness','Độ đầy đủ dữ liệu')]:`${x.completeness}%`,
      [bi(lang,'Penalty status','Trạng thái phạt')]:x.penaltyActive?bi(lang,'Active','Đang áp dụng'):bi(lang,'NOT APPLIED','CHƯA ÁP DỤNG'),
      [bi(lang,'Penalty amount','Mức phạt')]:x.penaltyActive?x.penaltyAmount:0
    }})
    const rubric=[
      {Metric:bi(lang,'Observation','Dự giờ'),Formula:bi(lang,'Official VMG observation score / 100','Điểm dự giờ VMG chính thức / 100'),Weight:'50%',Rubric:bi(lang,'>90 Excellent · 81–90 Very good · 70–80 Good · 60–69 Satisfactory · <60 Development/Training','>90 Xuất sắc · 81–90 Rất tốt · 70–80 Tốt · 60–69 Đạt · <60 Cần cải thiện/đào tạo')},
      {Metric:'HVR',Formula:bi(lang,'(Learners at start − dropouts) ÷ learners at start × 100','(HV đầu kỳ − HV rụng) ÷ HV đầu kỳ × 100'),Weight:'30%',Rubric:bi(lang,'Numeric retention rate; target thresholds remain configurable by policy.','Tỷ lệ duy trì dạng số; ngưỡng mục tiêu vẫn cấu hình theo chính sách.')},
      {Metric:bi(lang,'Revenue contribution','Doanh thu đóng góp'),Formula:bi(lang,`Index = revenue ÷ ${KPI_REVENUE_TARGET.toLocaleString()} × 100, capped at 100`,`Chỉ số = doanh thu ÷ ${KPI_REVENUE_TARGET.toLocaleString()} × 100, tối đa 100`),Weight:'20%',Rubric:bi(lang,'Current operating target is configurable; not a permanent policy constant.','Mốc vận hành hiện tại có thể cấu hình; không phải hằng số chính sách.')},
      {Metric:bi(lang,'Approved issues','Lỗi đã duyệt'),Formula:bi(lang,'Count of Director-approved cases in the month','Số sự vụ đã được GĐTT duyệt trong tháng'),Weight:'0%',Rubric:bi(lang,'Visible for transparency. Not deducted from KPI while penalty policy is inactive.','Hiển thị minh bạch. Chưa trừ KPI khi chính sách phạt chưa kích hoạt.')},
      {Metric:bi(lang,'KPI band','Xếp loại KPI'),Formula:'A ≥90 · B 80–89.9 · C 70–79.9 · D <70',Weight:'—',Rubric:bi(lang,'Draft operating rubric V1, subject to R&D/BOD approval.','Rubric vận hành V1, chờ R&D/BOD phê duyệt chính thức.')}
    ]
    const competencyMatrix=[
      {Domain:bi(lang,'Preparation & readiness','Chuẩn bị & sẵn sàng'),Evidence:bi(lang,'Lesson objectives, materials, readiness, programme alignment','Mục tiêu, học liệu, sự sẵn sàng, bám chương trình'),Rating1:bi(lang,'Needs support','Cần hỗ trợ'),Rating2:bi(lang,'Developing','Đang phát triển'),Rating3:bi(lang,'Meets standard','Đạt chuẩn'),Rating4:bi(lang,'Strong / role model','Mạnh / hình mẫu'),ObservationWeight:'10%'},
      {Domain:bi(lang,'Teaching methodology & skills','Phương pháp & kỹ năng giảng dạy'),Evidence:bi(lang,'Instruction, modelling, ICQ/CCQ, practice, feedback, assessment','Hướng dẫn, modelling, ICQ/CCQ, luyện tập, feedback, assessment'),Rating1:bi(lang,'Needs support','Cần hỗ trợ'),Rating2:bi(lang,'Developing','Đang phát triển'),Rating3:bi(lang,'Meets standard','Đạt chuẩn'),Rating4:bi(lang,'Strong / role model','Mạnh / hình mẫu'),ObservationWeight:'45%'},
      {Domain:bi(lang,'Classroom management','Quản lý lớp học'),Evidence:bi(lang,'Participation, routines, pace, engagement, differentiation','Participation, routines, pace, engagement, differentiation'),Rating1:bi(lang,'Needs support','Cần hỗ trợ'),Rating2:bi(lang,'Developing','Đang phát triển'),Rating3:bi(lang,'Meets standard','Đạt chuẩn'),Rating4:bi(lang,'Strong / role model','Mạnh / hình mẫu'),ObservationWeight:'45%'},
      {Domain:bi(lang,'Learner impact & retention','Tác động & duy trì học viên'),Evidence:bi(lang,'Learner base, dropouts, HVR, learner/service evidence','HV đầu kỳ, HV rụng, HVR, evidence HV/dịch vụ'),Rating1:bi(lang,'Needs support','Cần hỗ trợ'),Rating2:bi(lang,'Developing','Đang phát triển'),Rating3:bi(lang,'Meets standard','Đạt chuẩn'),Rating4:bi(lang,'Strong / role model','Mạnh / hình mẫu'),ObservationWeight:'—'},
      {Domain:bi(lang,'Professional conduct & growth','Tác phong & phát triển nghề nghiệp'),Evidence:bi(lang,'Approved cases, coaching, training, reflection, follow-through','Lỗi đã duyệt, coaching, training, reflection, follow-through'),Rating1:bi(lang,'Needs support','Cần hỗ trợ'),Rating2:bi(lang,'Developing','Đang phát triển'),Rating3:bi(lang,'Meets standard','Đạt chuẩn'),Rating4:bi(lang,'Strong / role model','Mạnh / hình mẫu'),ObservationWeight:'—'}
    ]
    const issueMatrix=[
      {Level:bi(lang,'Routine monitoring','Theo dõi bình thường'),Threshold:'0–1 / week',Action:bi(lang,'Routine monitoring; no automatic negative conclusion.','Theo dõi định kỳ; không tự động kết luận tiêu cực.'),Penalty:bi(lang,'Not applied','Chưa áp dụng')},
      {Level:bi(lang,'Watch','Theo dõi'),Threshold:'2 / week or 3 / month',Action:bi(lang,'Coaching + targeted observation / follow-up.','Coaching + dự giờ/follow-up có mục tiêu.'),Penalty:bi(lang,'Not applied','Chưa áp dụng')},
      {Level:bi(lang,'High risk','Rủi ro cao'),Threshold:'≥3 / week or ≥4 / month',Action:bi(lang,'Formal review + retraining + cautious allocation.','Đánh giá chính thức + đào tạo lại + thận trọng phân lớp.'),Penalty:bi(lang,'Not applied','Chưa áp dụng')},
      {Level:bi(lang,'Critical','Nghiêm trọng'),Threshold:bi(lang,'Any critical breach','Bất kỳ lỗi critical nào'),Action:bi(lang,'Immediate escalation and formal review.','Nâng mức xử lý ngay và đánh giá chính thức.'),Penalty:bi(lang,'Not applied','Chưa áp dụng')}
    ]
    const wb=XLSX.utils.book_new(),ws=XLSX.utils.json_to_sheet(rows),wr=XLSX.utils.json_to_sheet(rubric),wc=XLSX.utils.json_to_sheet(competencyMatrix),wi=XLSX.utils.json_to_sheet(issueMatrix)
    ws['!cols']=[10,28,14,12,10,16,14,12,15,10,18,14,14,14,12,20,18,18,14].map(w=>({wch:w}))
    wr['!cols']=[24,52,12,80].map(w=>({wch:w}))
    wc['!cols']=[34,62,20,20,20,22,18].map(w=>({wch:w}))
    wi['!cols']=[24,28,66,18].map(w=>({wch:w}))
    XLSX.utils.book_append_sheet(wb,ws,bi(lang,'Monthly KPI','KPI tháng'))
    XLSX.utils.book_append_sheet(wb,wr,bi(lang,'KPI rubric','Rubric KPI'))
    XLSX.utils.book_append_sheet(wb,wc,bi(lang,'Competency matrix','Ma trận năng lực'))
    XLSX.utils.book_append_sheet(wb,wi,bi(lang,'Issue thresholds','Ngưỡng sự vụ'))
    const scopeName=profile.role==='teacher'?(profile.teacher_code||'Teacher'):(profile.home_centre_code||profile.region_no?`Scope-${profile.home_centre_code||profile.region_no}`:'All-System')
    XLSX.writeFile(wb,`VMG_Teacher_KPI_${scopeName}_${month}.xlsx`)
  }
  ...
  function teacherIntel(teacher){
    ...
    const monthIntel=monthlyKpiIntel(teacher,nowDate().slice(0,7))
    ...
    return{score,hvr,weeklyCases:weekly,monthlyCases:monthly,risk,touchCount,docs:Number(docs),upgradePotential:potential,recommendation,kpiScore:monthIntel.score,kpiBand:monthIntel.band,kpiCompleteness:monthIntel.completeness,approvedCases:monthIntel.issues.approved,pendingCases:monthIntel.issues.pending,dropouts:monthIntel.dropouts,learnerStart:monthIntel.learnerStart,penaltyActive:monthIntel.penaltyActive,penaltyAmount:monthIntel.penaltyAmount}
  }
...
function KpiBandPill({band,score,completeness,lang}){const code=band?.code||'—';const tone=code==='A'?'green':code==='B'?'navy':code==='C'?'amber':code==='D'?'red':'muted';return <span className={`kpi-band-pill ${tone}`}><b>{code}{score!=null?` · ${score}`:''}</b><small>{band?(lang==='vi'?band.vi:band.en):bi(lang,'Insufficient data','Chưa đủ dữ liệu')} · {Number(completeness||0)}%</small></span>}
function KpiRubricMetric({code,title,weight,source,formula,detail}){return <article className="kpi-rubric-metric"><div className="kpi-rubric-code">{code}</div><div><span>{weight}</span><h4>{title}</h4><small>{source}</small><b>{formula}</b><p>{detail}</p></div></article>}
...
function RiskPill({risk,lang}){const rule=risk.level==='Stable'?bi(lang,'0–1 / week','0–1 / tuần'):risk.level==='Watch'?bi(lang,'2 / week or 3 / month','2 / tuần hoặc 3 / tháng'):risk.level==='Danger'?bi(lang,'≥3 / week or ≥4 / month','≥3 / tuần hoặc ≥4 / tháng'):bi(lang,'Any critical breach','Có lỗi critical');return <span className={`pill ${risk.tone==='stable'?'green':risk.tone==='watch'?'amber':'red'}`} title={riskAction(lang,risk)}>{rule}</span>}
...
function BulkImportCard({kind,meta,lang,onDone}){
  const [file,setFile]=useState(null),[busy,setBusy]=useState(false),[result,setResult]=useState(null),[error,setError]=useState(''),[progress,setProgress]=useState(null)
  async function readApiResponse(r){
    const text=await r.text()
    if(!text)return {}
    try{return JSON.parse(text)}catch{
      if(r.status===504)throw new Error(bi(lang,'The import took too long in one server request. Teacher 360 now imports large files in smaller batches; please retry after deploying this patch.','Lần nhập đã vượt thời gian xử lý của server. Teacher 360 đã được sửa để chia file lớn thành nhiều đợt; hãy thử lại sau khi deploy patch này.'))
      throw new Error(bi(lang,`The server returned an unreadable response (${r.status}). Please retry.`,`Server trả về phản hồi không đọc được (${r.status}). Vui lòng thử lại.`))
    }
  }
  async function submit(){
    if(!file)return
    const ok=confirm(bi(lang,`Import ${file.name}? Existing matching records may be updated or skipped.`,`Nhập file ${file.name}? Các bản ghi đã có có thể được cập nhật hoặc bỏ qua nếu trùng.`))
    if(!ok)return
    setBusy(true);setError('');setResult(null);setProgress({done:0,total:null})
    const combined={created:0,updated:0,skipped:0,failed:0,errors:[]}
    let offset=0,total=null
    const batchSize=kind==='users'?24:60
    try{
      while(true){
        const fd=new FormData();fd.append('kind',kind);fd.append('lang',lang);fd.append('file',file);fd.append('offset',String(offset));fd.append('batch_size',String(batchSize))
        const r=await fetch('/api/admin/import',{method:'POST',body:fd})
        const out=await readApiResponse(r)
        if(!r.ok)throw new Error(out.error||bi(lang,'Import failed','Nhập dữ liệu thất bại'))
        combined.created+=Number(out.created||0);combined.updated+=Number(out.updated||0);combined.skipped+=Number(out.skipped||0);combined.failed+=Number(out.failed||0)
        if(out.errors?.length)combined.errors.push(...out.errors)
        total=Number(out.total||total||0);offset=Number(out.next_offset??(offset+Number(out.processed||0)))
        setProgress({done:Math.min(offset,total||offset),total:total||null})
        if(!out.has_more)break
      }
      combined.errors=combined.errors.slice(0,60)
      setResult(combined);onDone?.()
    }catch(e){setError(e.message)}finally{setBusy(false)}
  }
  const busyLabel=progress?.total?bi(lang,`Importing ${progress.done}/${progress.total}…`,`Đang nhập ${progress.done}/${progress.total}…`):bi(lang,'Importing…','Đang nhập…')
  return <article className="import-card"><div className="import-card-top"><span className="import-number">{meta.icon}</span><div><span className="import-eyebrow">{t(lang,'bulkImport')}</span><h3>{meta.title}</h3></div></div><p>{meta.text}</p><div className="import-columns"><b>{bi(lang,'Expected columns','Cột dữ liệu')}</b><span>{meta.columns}</span></div><div className="drop-zone"><input aria-label={t(lang,'chooseFile')} type="file" accept=".csv,.xlsx,.xls" onChange={e=>{setFile(e.target.files?.[0]||null);setResult(null);setError('');setProgress(null)}}/><span className="drop-icon">⇩</span><b>{file?.name||t(lang,'chooseFile')}</b><small>{bi(lang,'Up to 1,000 rows per import','Tối đa 1.000 dòng mỗi lần')}</small></div><div className="import-actions"><button className="btn secondary" type="button" onClick={()=>downloadImportTemplate(kind,'csv')}>↓ CSV</button><button className="btn secondary" type="button" onClick={()=>downloadImportTemplate(kind,'xlsx')}>↓ Excel</button><button className="btn primary" type="button" disabled={!file||busy} onClick={submit}>{busy?busyLabel:t(lang,'startImport')} →</button></div>{busy&&progress?.total&&<div className="import-progress" aria-live="polite"><i style={{width:`${Math.round((progress.done/progress.total)*100)}%`}}/><span>{Math.round((progress.done/progress.total)*100)}%</span></div>}{error&&<div className="import-result error"><b>{t(lang,'failed')}</b><span>{error}</span></div>}{result&&<div className="import-result success"><b>{t(lang,'importSummary')}</b><div><span>{t(lang,'created')} <strong>{result.created}</strong></span><span>{t(lang,'updated')} <strong>{result.updated}</strong></span><span>{t(lang,'skipped')} <strong>{result.skipped}</strong></span><span>{t(lang,'failed')} <strong>{result.failed}</strong></span></div>{result.errors?.length>0&&<details><summary>{bi(lang,'Show row errors','Xem lỗi từng dòng')}</summary>{result.errors.map((e,i)=><p key={i}>{bi(lang,'Row','Dòng')} {e.row}: {e.message}</p>)}</details>}</div>}</article>
}
...
function KpiForm({teachers,lang,defaultMonth,onSave}){
  const baseMonth=defaultMonth||new Date().toISOString().slice(0,7)
  const [y,m]=baseMonth.split('-').map(Number),lastDay=new Date(y,m,0).getDate()
  const start=`${baseMonth}-01`,end=`${baseMonth}-${String(lastDay).padStart(2,'0')}`
  const [f,setF]=useState({teacher_id:teachers[0]?.id||'',period_start:start,period_end:end,observation_score:'',learner_start_count:'',learner_dropout_count:'',revenue_contribution:'',notes:''})
  const hvr=retentionPct(Number(f.learner_start_count||0),Number(f.learner_dropout_count||0))
  return <form onSubmit={e=>{e.preventDefault();onSave(f)}}>
    <div className="kpi-form-callout"><b>{bi(lang,'Enter raw monthly data — Teacher 360 calculates the rates.','Nhập dữ liệu gốc theo tháng — Teacher 360 tự tính các tỷ lệ.')}</b><span>{bi(lang,'For new records, do not type HVR manually. Enter learner base and dropouts so the calculation can be audited later.','Với dữ liệu mới, không nhập HVR thủ công. Hãy nhập số HV đầu kỳ và số HV rụng để sau này có thể kiểm tra lại cách tính.')}</span></div>
    <div className="form-grid two"><Field label={bi(lang,'Teacher','Giáo viên')}><select value={f.teacher_id} onChange={e=>setF({...f,teacher_id:e.target.value})}>{teachers.map(x=><option key={x.id} value={x.id}>{x.full_name} · {x.home_centre_code||'VMG'}</option>)}</select></Field><Field label={bi(lang,'Observation score /100 (optional)','Điểm dự giờ /100 (không bắt buộc)')}><input type="number" min="0" max="100" step="0.1" value={f.observation_score} onChange={e=>setF({...f,observation_score:e.target.value})}/><small className="field-help">{bi(lang,'Leave blank to use the average official observation score recorded in this month.','Để trống để hệ thống lấy trung bình điểm dự giờ chính thức trong tháng.')}</small></Field><Field label={bi(lang,'Learners at start of period','Số học viên đầu kỳ')}><input required type="number" min="0" step="1" value={f.learner_start_count} onChange={e=>setF({...f,learner_start_count:e.target.value})}/></Field><Field label={bi(lang,'Learner dropouts in period','Số học viên rụng trong kỳ')}><input required type="number" min="0" step="1" max={Number(f.learner_start_count||0)||undefined} value={f.learner_dropout_count} onChange={e=>setF({...f,learner_dropout_count:e.target.value})}/></Field><Field label="HVR"><div className="calculated-field"><b>{hvr!=null?`${hvr}%`:'—'}</b><small>{bi(lang,'Automatically calculated: retained ÷ learner base','Tự động tính: HV còn lại ÷ HV đầu kỳ')}</small></div></Field><Field label={bi(lang,'Revenue contribution (VND)','Doanh thu đóng góp (VND)')}><input type="number" min="0" value={f.revenue_contribution} onChange={e=>setF({...f,revenue_contribution:e.target.value})}/><small className="field-help">{bi(lang,'Optional until the business KPI policy is finalised.','Có thể để trống cho đến khi chính sách KPI kinh doanh được chốt.')}</small></Field><Field label={bi(lang,'Period start','Bắt đầu kỳ')}><input type="date" value={f.period_start} onChange={e=>setF({...f,period_start:e.target.value})}/></Field><Field label={bi(lang,'Period end','Kết thúc kỳ')}><input type="date" value={f.period_end} onChange={e=>setF({...f,period_end:e.target.value})}/></Field></div>
    <div style={{marginTop:12}}><Field label={bi(lang,'Monthly notes / data source','Ghi chú tháng / nguồn dữ liệu')}><textarea value={f.notes} onChange={e=>setF({...f,notes:e.target.value})} placeholder={bi(lang,'Optional: source, exceptions, learner transfer notes…','Không bắt buộc: nguồn dữ liệu, ngoại lệ, ghi chú chuyển lớp…')}/></Field></div>
    <div className="friendly-note" style={{margin:'13px 0 0'}}><b>{bi(lang,'Calculation currently used','Cách tính hiện đang dùng')}</b><p>{bi(lang,'Observation 50% · HVR 30% · Revenue contribution 20%. Missing metrics are not treated as zero; available weights are re-normalised and Data Completeness shows how much evidence is present. Approved issue counts are recorded separately and do not deduct KPI because the penalty policy is not active.','Dự giờ 50% · HVR 30% · Doanh thu đóng góp 20%. Metric chưa có dữ liệu không bị tính là 0; hệ thống tái chuẩn hóa theo trọng số đang có và hiển thị Độ đầy đủ dữ liệu. Số lỗi đã duyệt được ghi riêng và chưa trừ KPI vì cơ chế phạt chưa áp dụng.')}</p></div><FormActions lang={lang}/></form>
}
function UserForm({lang,onSave,initialUser=null}){
  const editMode=Boolean(initialUser?.id)
  const base={id:'',email:'',password:'',full_name:'',role:'teacher',staff_code:'',job_title:'',teacher_code:'',home_centre_code:'PVT',region_no:1,professional_level:'',language_preference:'en',is_active:true}
  const initial=initialUser?{...base,id:initialUser.id,email:initialUser.email||'',full_name:initialUser.full_name||'',role:initialUser.role||'teacher',staff_code:initialUser.staff_code||'',job_title:initialUser.job_title||'',teacher_code:initialUser.teacher_code||'',home_centre_code:initialUser.home_centre_code||'',region_no:initialUser.region_no||'',professional_level:initialUser.professional_level||'',language_preference:initialUser.language_preference||'en',is_active:initialUser.is_active!==false}:base
  ...
  return <form onSubmit={submit}>
    ...
    <div className="form-grid two">
      ...
      <Field label={editMode?bi(lang,'New password (optional)','Mật khẩu mới (không bắt buộc)'):t(lang,'temporaryPassword')}><div className="password-field"><input required={!editMode} type="text" minLength="8" value={f.password} onChange={e=>setF({...f,password:e.target.value})}/><button type="button" onClick={()=>setF({...f,password:`VMG!${Math.random().toString(36).slice(2,8)}${Math.floor(10+Math.random()*89)}`})}>{bi(lang,'Generate','Tạo nhanh')}</button></div></Field>
      ...
      {editMode&&<Field label={t(lang,'status')}><select value={f.is_active?'active':'inactive'} onChange={e=>setF({...f,is_active:e.target.value==='active'})}><option value="active">{bi(lang,'Active','Đang hoạt động')}</option><option value="inactive">{bi(lang,'Inactive / blocked','Ngưng hoạt động / khóa')}</option></select></Field>}
    </div>
...
function CalendarSyncChip({lang,status,onRetry}){const st=status||'pending';if(st==='synced')return <span className="calendar-sync synced">✓ {bi(lang,'Synced','Đã đồng bộ')}</span>;if(st==='syncing')return <span className="calendar-sync syncing">↻ {bi(lang,'Syncing','Đang đồng bộ')}</span>;return <button type="button" className={`calendar-sync ${st==='failed'?'failed':'pending'}`} onClick={onRetry}>{st==='failed'?'!':'↗'} {bi(lang,st==='failed'?'Retry':'Add to Calendar',st==='failed'?'Thử lại':'Thêm vào lịch')}</button>}
